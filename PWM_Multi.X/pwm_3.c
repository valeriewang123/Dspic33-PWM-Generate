/*******************************************************************************
  ...
  Description:
        This is main file for generating two pwm signals of different frequencies
        generated by Timer2 and Timer4. 
        User can modify the Period and Duty cycle as desired. Pulse period 
        can be modified by writting to PR2 registe. Duty cycle can be 
        modified by writing to respective OCxR registers. 
 *******************************************************************************/
...
void Init_CLK( void );
void Init_PPS( void ); // Peripheral Pin Selection Function: configure the output of PWM signal
void Init_OC1( void ); 
...
int main(void)
{
    ...
    Init_Timer4 ( );        // Initilise Timer 4 with Period = 2100/70Mhz = 30us
    Init_PPS();             // Map OC1 Ouputs to RP43R
    Init_OC1();             // Initialise OC1 module with duty cycle = 50%
    Init_OC2();             // Initialise OC2 module with duty cycle = 33%
    T2CONbits.TON = 1;      // Start the timer,the clock source for Output Compare Modules
    T4CONbits.TON = 1;      // Start the timer,the clock source for Output Compare Modules
...
/******************************************************************************
*   By setting PR4 = 2100
*   Pluse Period = 2100/Fout = 2100/70Mhz = 30us 
*   Reference: http://ww1.microchip.com/downloads/en/DeviceDoc/S11.pdf
*   11.3 - CONTROL REGISTERS - From Page 11-6
 *****************************************************************************/
void Init_Timer4( void )
{
    T4CON = 0X0000;
    TMR4 = 0X0000;
    PR4 = 2100; // Specify the Pulse period here,is configured for 30 us
}
...
void Init_OC1( void )
{
 ...
 void Init_OC2( void )         // More PWM generators
 {
  OC2R = 700;                 // Specify the pulse width of the OC2 Output ,initialised for 10us width, namely 33% Duty Cycle
  OC2RS = 0x000;              // Initialize the secondary compare register
  OC2CON1 = 0x0;              // Clear all control bits
  OC2CON2 = 0x0;              // Clear all control bits
  OC2CON1bits.OCTSEL = 0x0;   // Select peripheral clock as clock source
  //http://elecanisms.olin.edu/frm/35_Output_Compare.pdf 
  //set OCXCON2, select Timer x as source
  OC2CON2bits.SYNCSEL = 0xE;  // Select Timer4 Output as sync source
...
void Init_PPS( void )
{
    ...
    _RP42R = 0X11;  // RP42 = RB10 pin is mapped as OC2 out
    __builtin_write_OSCCONL( OSCCON | (1 << 6) );
}
